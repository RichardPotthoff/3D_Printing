...
tol=0.15;
number_of_planets=3;
number_of_teeth_on_planets=20;
approximate_number_of_teeth_on_sun=19;
// pressure angle
P=30;//[30:60]
...
echo(ns);
nr=ns+2*np;
pitchD=0.85*D/(1+min(PI/(2*nr*tan(P)),PI*DR/nr));
pitch=pitchD*PI/nr;
echo(pitch);
Dp=pitch*(nr+ns)/2;
echo(Dp);
...
translate([0,0,T/2]){
	...
	difference(){
		...
		cylinder(r=w/sqrt(3),h=T+1,center=true,$fn=nh);
	}
