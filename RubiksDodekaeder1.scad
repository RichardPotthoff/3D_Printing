...
module template(offset,D_sphere,d_2,h_step)
  {
  translate([0,0,offset+D_sphere/2])
    ...
    union()
      ...
      intersection()
        ...
        translate([0,0,-offset-D_sphere/2])sphere(r=d_2/2,$fn=120);
        }
...
#mirror([0,0,1])template(offset=5,D_sphere=50,d_2=26,h_step=2);
rotate([0,0,72*1])
rotate([116.565,0,0])
template(offset=5,D_sphere=50,d_2=26,h_step=2);
#rotate([0,0,72*2])
rotate([116.565,0,0])
template(offset=5,D_sphere=50,d_2=26,h_step=2);
rotate([0,0,72*3])
...
module template1()template(offset=6,D_sphere=60,d_2=27,h_step=2);
...
difference()
  ...
  translate([0,0,-16.7])
    {
    cylinder(r=3.1/2,h=10,$fn=16);
    cylinder(r=5.6/2,h=3.0,$fn=16);
    translate([0,0,2.99])cylinder(r1=5.6/2,r2=3.1/2,h=(5.6-3.1)/2,$fn=16);
    }
  }
translate([10,5,0])cornerPiece();
translate([5,10,0])edgePiece();
translate([0,0,10])dodecahedronWithHoles(height=20,d_outer_sphere=1.17*20,d_hole=2.5,d_inner_sphere=14);